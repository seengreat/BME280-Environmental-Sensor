...
int main(int argc, char* argv[])
{
    ...
    if (ioctl(id.fd, I2C_SLAVE, BME280_I2C_ADDR_SEC) < 0)
    {
    ...
    {
        printf("ioctl i2c dev success:%d\r\n",id.dev_addr);
    }
    ...
    {
        printf("wiringpi setup init OK!\r\n");    
    }
    if(wiringPiSPISetup(SPI_CH,2000000)<0)
    {
        
    }
    else
    {
        printf("wiringpi spi init OK!\r\n");         
    }
    pinMode(PIN_CS,OUTPUT);
    digitalWrite(PIN_CS,LOW);//select the SPI interface
    /* Make sure to select BME280_I2C_ADDR_PRIM or BME280_I2C_ADDR_SEC as needed */
    id.dev_addr = 0;
    id.fd = 0;
...
int8_t user_spi_read(uint8_t reg_addr, uint8_t *data, uint32_t len, void *intf_ptr)
{
    ...
    wiringPiSPIDataRW(SPI_CH,&reg_addr,1);
...
int8_t stream_sensor_data_normal_mode(struct bme280_dev *dev)
{
    ...
    dev->settings.standby_time = BME280_STANDBY_TIME_125_MS; //controls inactive duration stanby in normal mode.
